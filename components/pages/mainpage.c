...
static void remote_state_task(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        // 检查标签是否仍然有效
        if(!lv_obj_is_valid(keys_state_label) || !lv_obj_is_valid(battery_label) || !lv_obj_is_valid(rocker_label) || !lv_obj_is_valid(btn_label))
        {
            // 标签无效，跳过UI更新
            vTaskDelay(update_interval);
            continue;
        ...
        if(lv_obj_is_valid(keys_state_label) && lv_obj_is_valid(battery_label) && lv_obj_is_valid(rocker_label) && lv_obj_is_valid(btn_label))
        {
            ...
            // 查找最小位的按键编号（从bit0开始，bit0为1则显示1，bit1为1则显示2，以此类推）
            uint8_t btn_num = 0;
            if(remote_key != 0)
            {
                for(int i = 0; i < 16; i++)
            ...
            char btn_str[16]={0};
            sprintf(btn_str,"Btn:%d",btn_num);
            lv_label_set_text(btn_label, btn_str);
...
void main_page_create(void *user_data)
{
    ...
    btn_label = lv_label_create(lv_screen_active());
    lv_label_set_text(btn_label, "Btn:0");
    lv_obj_align(btn_label, LV_ALIGN_TOP_MID, 0, 25);
    set_remote_flush_func(main_page_remote_state_flush_func,btn_label);
